...
def load_saved_artifacts():
    ...
    with open("./artifacts/class_dictionary_celebrity.json", "r") as f:
        __class_name_to_number = json.load(f)
        __class_number_to_name = {v:k for k,v in __class_name_to_number.items()}

    global __model
    if __model is None:
        with open('./artifacts/saved_mode_celebrity.pkl', 'rb') as f:
            __model = joblib.load(f)
    print("loading saved artifacts...done")


def get_cv2_image_from_base64_string(b64str):#either we can convert image to encoded base string or use cloud server storage s3 to retrieve image
    '''
